...
Visualize simplified wind flow at street level, highlighting danger zones where wind accelerates between tall buildings (Venturi effect). Uses Kingston's prevailing W/SW wind direction.
...
**Base wind:**
Kingston's prevailing wind is from the W/SW. Default: 15 km/h (4.2 m/s) from 250 degrees (WSW).

```typescript
const KINGSTON_PREVAILING_WIND = {
