...
Parking estimation by zone type (Kingston By-Law parking minimums):
...
Lot area estimation (if not provided): `buildingFootprint * 2.5` (typical lot-to-building ratio for urban Kingston).
...
Curve Numbers for Kingston conditions (Hydrologic Soil Group B — typical for Kingston's clay-loam):
| Surface | CN |
|---|---|
...
Design storms for Kingston (IDF curves):
...
| `lib/kingstonZoning.ts` | Map zone codes to parking requirements and pre-development land cover |
...
- Kingston IDF curves: Environment and Climate Change Canada IDF data for Kingston (Station 6104025)
- Parking minimums: City of Kingston Zoning By-Law 2022-62
